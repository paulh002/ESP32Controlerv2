...
bool FT891_CAT::ProcessCmd ()
{
	...
	switch ( newMessage.ID )							// Make decisions based on message ID
	{
		...
		case MSG_BS:
			...
			radioStatus.BND = tempBND;
			cmdProcessed = true;
			if (!bVFOmode)
				catcommunicator_->setband(radioStatus.BND);
			break;
...
//			radioStatus.IS_STAT = dataBuff[0] - '0';	// Set on/off status
		...
		case MSG_SH:
			strncpy ( tempBuff, &dataBuff[1], 5 );		// Extract the offset amount
			radioStatus.SH_VALUE = (int32_t)atoi ( tempBuff );	// Set value in the structure
...
//			radioStatus.RM = atoi ( tempBuff );
		...
		case MSG_GT:									// Get information command
			...
			{
				catcommunicator_->Processinformation(dataBuff);
			}
			cmdProcessed = true;
			break;
...
uint8_t FT891_CAT::GetAG(bool request)							// Get volume
{
...
uint32_t FT891_CAT::GetSH()							// Get IF bandwidth
{
